...
export default function DashboardPage() {
  ...
  const { application, applications } = useApplicationState();
  const { user } = useAuth();
  ...
  const notificationItems = useMemo(
    () => [
        ...
        ? [
            {
              ...
              title: `${service?.name ?? "Your application"} is ${statusLabels[application.status].toLowerCase()}`,
      ...
      ...(application && ["draft", "ready_for_payment"].includes(application.status)
        ...
        : []),
      ...(application &&
      documents.some((doc) => doc.required && doc.status === "required")
        ? [
            {
              id: "documents",
  ...
  const stats = [
    ...
    },
    {
      ...
      value: applications.filter((item) =>
        ["ready_for_payment", "draft"].includes(item.status),
      ).length,
      icon: CreditCard,
    },
    {
    ...
    <div className="min-h-screen bg-[var(--fm-graphite)] text-[var(--fm-text-primary)]">
      ...
      >
        <div className="relative flex h-[69px] shrink-0 items-center border-b border-[var(--fm-border)] px-4">
          ...
          >
            <span className="text-[28px] font-semibold tracking-[-1.7px]">
              audvertax
            </span>
      ...
      >
        <header className="sticky top-0 z-30 flex h-[53px] items-center justify-between border-b border-[var(--fm-border)] bg-[var(--fm-surface)] px-4 sm:px-6">
          ...
          <div className="flex items-center gap-4 text-[var(--fm-text-secondary)]">
            ...
            <div className="flex size-8 items-center justify-center rounded-full border border-[var(--fm-border)] bg-[var(--fm-surface-raised)] font-mono text-[11px] font-bold text-[var(--fm-lime)]">
        ...
        <main className="mx-auto max-w-[1120px] px-4 py-8 sm:px-6 lg:px-8">
          {active === "Dashboard" && (
            <>
              <PageHeader
                ...
                description="Here's an overview of your LLC portfolio and pending actions."
                className="mb-7"
              />
          ...
          {active === "Support" && (
            <DashboardSupport faqOpen={faqOpen} setFaqOpen={setFaqOpen} />
          )}
          {active === "Settings" && (
            <DashboardSettings
              name={name}
...
function DashboardBilling({ application, service, pricing }: any) {
  if (!application) {
    return (
      <Empty
        title="Billing"
        text="Start an application to see billing information."
      />
    );
  }

  const paymentReady =
    application.status === "ready_for_payment" || application.status === "draft";
  ...
  return (
    <>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
        <DashboardPanel
          title="Current order"
          description={service?.name ?? application.serviceSlug}
          action={
        ...
        >
          <div>
            ...
            <div className="mt-5 flex justify-between">
              <span className="text-sm text-[var(--fm-text-tertiary)]">
                Total
              </span>
              <b className="text-xl">
      ...
      >
        {items.length ? (
          ...
          <div className="py-10 text-center">
            <Bell
              className="mx-auto text-[var(--fm-text-tertiary)]"
              size={38}
            />
    ...
    <div className="absolute right-0 top-10 z-50 w-[350px] overflow-hidden rounded-[var(--fm-radius-lg)] border border-[var(--fm-border)] bg-[var(--fm-surface)] shadow-[var(--fm-shadow-elevated)]">
      <div className="flex items-center justify-between border-b border-[var(--fm-border)] px-4 py-3">
        ...
        {unreadCount > 0 && (
          <button
            onClick={markAllRead}
            className="text-[10px] font-semibold text-[var(--fm-lime)]"
          >
            Mark read
          </button>
        )}
      ...
      <div className="max-h-[360px] overflow-auto">
        ...
        {!items.length && (
          <p className="p-6 text-center text-xs text-[var(--fm-text-tertiary)]">
            No notifications
          </p>
        )}
      </div>
          ...
          {faqs.map(([question, answer], index) => (
            <div key={question}>
              ...
              >
                ...
                <ChevronDown
                  size={17}
                  className={faqOpen === index ? "rotate-180" : ""}
                />
              </button>
              {faqOpen === index && (
