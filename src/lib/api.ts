...
export async function apiRequest<T>(path: string, options: RequestInit = {}): Promise<T> { const response = await fetch(`${API_URL}${path}`, { ...options, credentials: "include", headers: { "Content-Type": "application/json", ...(options.headers ?? {}) } }); const data = await response.json().catch(() => null); if (!response.ok) throw new Error(data?.error?.message ?? data?.message ?? "Something went wrong. Please try again."); return data as T; }
export type AuthUser = { id: string; email: string; firstName: string; lastName: string; role: "customer" | "admin" | "staff"; emailVerified?: boolean; authProvider?: string; googleSubject?: string | null; createdAt?: string; updatedAt?: string };
export type ApplicationStatus = "draft" | "in_review" | "ready_for_payment" | "paid" | "processing" | "completed" | "cancelled";
export type ApplicationMember = { id: string; fullName: string; country?: string; dateOfBirth?: string; ownershipPercentage?: number; address?: string };
export type ApplicationDocumentReference = { id: string; documentType: string; ownerType: "application" | "member"; ownerId?: string; fileName: string; storageKey: string; status: "uploaded" | "requested" | "approved" | "rejected"; createdAt: string; updatedAt: string };
export type Application = { id: string; userId: string; serviceSlug: string; packageSlug?: string; formationState?: string; variantSlug?: string; addOnSlugs?: string[]; members: ApplicationMember[]; documents: ApplicationDocumentReference[]; currentStep: number; answers: Record<string, unknown>; status: ApplicationStatus; createdAt: string; updatedAt: string };
...
type AuthResponse = { success: true; data: { user: AuthUser } & Record<string, unknown> }; type ApplicationResponse = { success: true; data: { application: Application } }; type ApplicationsResponse = { success: true; data: { applications: Application[] } }; type BillingResponse = { success: true; data: BillingOrder | null };
...
export async function forgotPassword(email: string) { return apiRequest<{ success: true; data: { message?: string } }>("/api/v1/auth/forgot-password", { method: "POST", body: JSON.stringify({ email }) }); }
export async function resetPassword(input: { email?: string; token?: string; password: string }) { return apiRequest<{ success: true; data: { message?: string; user?: AuthUser } }>("/api/v1/auth/reset-password", { method: "POST", body: JSON.stringify(input) }); }
export async function updatePassword(input: { currentPassword?: string; newPassword: string; confirmPassword?: string }) { return apiRequest<{ success: true; data: { message?: string } }>("/api/v1/auth/update-password", { method: "POST", body: JSON.stringify(input) }); }
