...
export default function PageTransition({ children }: { children: ReactNode }) {
  ...
  const revealWhenReady = useCallback(async (transition: ActiveTransition) => {
    const minimumDuration = wait(Math.max(0, PAGE_TRANSITION_CONFIG.minDuration - (Date.now() - transition.startedAt)));
  ...
  const startTransition = useCallback((url: URL, navigate: Navigate) => {
  ...
  useEffect(() => {
    ...
    if (transition && phaseRef.current === "covering" && previousPathname !== pathname && pathname === transition.destinationPathname) {
  ...
  useEffect(() => {
    const handlePopState = () => {
      ...
      if (phaseRef.current !== "idle" || nextUrl.pathname === pathnameRef.current || !isTransitionRoute(nextUrl.pathname) || isReducedMotion()) return;
  ...
  return (
    <>
      ...
      {phase !== "idle" && (
        <div aria-hidden="true" className={`page-transition-curtain page-transition-curtain--${phase}`}>
          <span className="page-transition-curtain__mark" />
          {isWaitingForDestination && phase === "covering" && <span className="page-transition-curtain__status">Loading destination</span>}
        </div>
      )}
    </>
  );
