...
const FEATURES = [ 
  ...
  { icon: FolderLock, title: "Every document in one place", desc: "Sub-Material to company formation and filing." },
  { icon: LineChart, title: "Full payment visibility ", desc: "Every payment of your process, tracked under one dashboard." },
];
      ...
      <div className="relative z-[1] mx-auto max-w-[1100px]" style={{ perspective: 1200 }}>
        <Card variant="feature" tone="dark" className="overflow-hidden p-0">
          <div className="flex items-center gap-1.5 border-b border-[var(--fm-border)] bg-[var(--fm-surface)] px-4 py-2.5">
            ...
            <span className="ml-3 rounded-[var(--fm-radius-sm)] border border-[var(--fm-border)] bg-[var(--fm-surface-raised)] px-3.5 py-[3px] font-mono text-[11.5px] font-medium text-[var(--fm-text-tertiary)]">app.audvertax.pk</span>
          </div>
          <div className="relative w-full" style={{ aspectRatio: "16 / 9.8" }}>
            <Image src="/audvertax_customer_dashboard.svg" alt="Audvertax customer dashboard" fill sizes="(max-width: 768px) 100vw, (max-width: 1280px) 90vw, 1100px" className="object-cover object-left-top" />
          </div>
        </Card>
      </div>
      ...
      <div className="relative z-[1] mx-auto mt-12 grid max-w-[1100px] grid-cols-1 gap-[18px] sm:grid-cols-2 lg:grid-cols-4">
        {FEATURES.map(({ icon: Icon, title, desc }) => (
          <Card key={title} variant="interactive" className="flex items-start gap-3.5 p-5">
            <IconContainer className="size-[38px] rounded-[var(--fm-radius-sm)]" aria-hidden="true"><Icon className="h-[18px] w-[18px]" /></IconContainer>
            <div><p className="mb-1 font-sans text-sm font-extrabold text-[var(--fm-card-text)]">{title}</p><p className="text-[13px] font-medium leading-relaxed text-[var(--fm-card-muted)]">{desc}</p></div>
          </Card>
        ))}
      </div>
    </section>
