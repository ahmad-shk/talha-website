...
const CARDS: CardData[] = [
  { title: "USA", quote: "MISSING DEADLINES FEARS, FEARING IRS PENALTIES", solution: "AudVertax providing complete US corporate and non-resident filing. We handle IRS returns and state annual reports.", tint: "blue", rotate: -3 },
  { title: "United Kingdom (HMRC)", quote: "Stuck on HMRC & Identity Verification", solution: "AudVertax providing end-to-end UK tax returns, Corporation Tax (CT600), VAT filings, and confirmation statements tailored for global directors and non-resident founders.", tint: "violet", rotate: 2 },
  { title: "United Arab Emirates (FTA)", quote: "Confused by UAE Corporate Tax & VAT Thresholds", solution: "FTA-compliant Corporate Tax filing, registration, and VAT management for Free Zone and Mainland entities, keeping your offshore or onshore operations fully shielded.", tint: "peach", rotate: -2 },
  { title: "Pakistan (FBR)", quote: "FBR Compliance & Cross-Border Remittances", solution: "Precision FBR income tax returns, ATL restoration, and foreign source income documentation, ensuring seamless profit repatriation and zero tax audit friction.", tint: "blue", rotate: 3 },
  { title: "MULTI JURISDICTION", quote: "Drowning in Double Taxation Risks", solution: "Strategic international tax structuring and treaty relief application. We align your US, UK, UAE, and Pakistan filings to legally minimize global tax liabilities.", tint: "violet", rotate: -3 },
  { title: "Fragmented Advisory", quote: "Too Many Local Tax Agents to Manage", solution: "One unified tax partner. Access direct WhatsApp communication, single-point account management, and standardized global tax filing across all your active jurisdictions.", tint: "peach", rotate: 2 },
];
  ...
  return (
    <section aria-labelledby="ps-heading" className="relative flex w-full flex-col items-center overflow-hidden bg-[var(--fm-graphite)] px-4 py-24 text-[var(--fm-text-primary)]">
        ...
        {CARDS.map((card) => {
          ...
          return (
            ...
            >
              ...
              <h3 className="font-display text-[22px] font-bold leading-tight text-[var(--fm-card-interactive-title)]" style={{ letterSpacing: "-0.3px" }}>{card.title}</h3>
              <p className="mt-2 font-sans text-[13.5px] font-semibold italic leading-snug text-[var(--fm-card-interactive-quote)]">{card.quote}</p>
              <p className="mt-2 font-sans text-sm leading-relaxed text-[var(--fm-card-interactive-body)]">{card.solution}</p>
            </Card>
          );
        })}
      </div>
      <div className="relative z-[2] mt-16 flex items-center gap-3">
        ...
        <p className="text-left font-sans text-[15px] leading-snug text-[var(--fm-text-secondary)]"><span className="text-[21px] font-bold text-[var(--fm-lime)]" style={{ fontFamily: "var(--font-caveat), cursive" }}>Your all in</span><br />Global setup partner.</p>
      </div>
    </section>
  );
}
