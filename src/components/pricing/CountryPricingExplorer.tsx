...
function NodeList({
  ...
  return (
    <div className={depth > 0 ? "mt-2 space-y-2 border-l border-[var(--fm-border)] pl-3" : "space-y-2"}>
      {nodes.map((node) => {
          ...
          <div key={node.title}>
            {node.serviceSlug ? (
              <button
                ...
                className={["group flex w-full items-center justify-between gap-3 rounded-[var(--fm-radius-md)] border px-4 py-3 text-left transition-[background-color,border-color,transform] duration-[var(--fm-motion-micro)] ease-[var(--fm-motion-ease)]", selected ? "border-[var(--fm-lime)]/45 bg-[var(--fm-lime)]/[.10]" : "border-[var(--fm-border)] bg-[var(--fm-surface)] hover:-translate-y-px hover:border-[var(--fm-border-accent)] hover:bg-[var(--fm-surface-raised)]"].join(" ")}
              >
                <span className="flex min-w-0 items-center gap-3">
                  ...
                  <span className="truncate text-sm font-semibold text-[var(--fm-text-primary)]">{node.title}</span>
                </span>
              ...
              <div className="flex items-center gap-3 rounded-[var(--fm-radius-md)] border border-[var(--fm-border)] bg-[var(--fm-surface-raised)] px-4 py-3">
                ...
                <span className="text-sm font-bold text-[var(--fm-text-primary)]">{node.title}</span>
              </div>
            )}
  ...
  return (
    <Card variant="elevated" className="h-fit overflow-hidden">
      <div className="border-b border-[var(--fm-card-divider)] p-6 sm:p-8">
        ...
        <h2 className="mt-3 font-display text-3xl font-extrabold tracking-[-.045em] text-[var(--fm-card-text)]">{service.name}</h2>
      ...
      {packages.length > 0 ? (
            ...
            <div key={item.slug} className="p-6 sm:p-8">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between sm:gap-5">
                <div>
                  ...
                  <p className="mt-2 max-w-[470px] text-sm leading-6 text-[var(--fm-card-muted)]">{item.description}</p>
                </div>
                <div className="shrink-0 text-right">
                  <div className="font-display text-2xl font-extrabold tracking-[-.04em] text-[var(--fm-card-text)]">{formatPrice(item.price, item.currency)}</div>
                  {item.price > 0 && <div className="mt-1 font-mono text-[9px] uppercase tracking-[.12em] text-[var(--fm-card-muted)]">One-time</div>}
                </div>
              </div>
              <div className="mt-5 grid gap-2 sm:grid-cols-2">
                {item.features.map((feature) => (
                  <div key={feature} className="flex items-start gap-2 text-xs leading-5 text-[var(--fm-card-muted)]">
              ...
              <Link href={getServiceHref(service.slug)} className="mt-6 inline-flex items-center gap-2 rounded-[var(--fm-radius-pill)] bg-[var(--fm-lime)] px-5 py-3 text-sm font-bold text-[var(--fm-graphite-deep)] transition-transform duration-[var(--fm-motion-micro)] hover:-translate-y-px hover:bg-[var(--fm-lime-bright)]">
        ...
        <div className="p-6 sm:p-8">
          <div className="rounded-[var(--fm-radius-lg)] border border-dashed border-[var(--fm-border-accent)] bg-[var(--fm-lime)]/[.05] p-5">
            <p className="font-mono text-[10px] font-bold uppercase tracking-[.14em] text-[var(--fm-lime)]">Pricing status</p>
            <p className="mt-2 text-sm leading-6 text-[var(--fm-card-muted)]">This service is in the catalog, but a public package price has not been configured yet. No price has been invented here.</p>
          </div>
          <Link href={getServiceHref(service.slug)} className="mt-6 inline-flex items-center gap-2 rounded-[var(--fm-radius-pill)] border border-[var(--fm-border)] bg-[var(--fm-surface)] px-5 py-3 text-sm font-bold text-[var(--fm-text-primary)] hover:border-[var(--fm-border-accent)] hover:bg-[var(--fm-surface-raised)]">
...
export default function CountryPricingExplorer() {
  ...
  return (
    <main className="fm-page overflow-hidden">
      ...
      <section className="px-fm-5 py-fm-20 sm:px-fm-8 sm:py-fm-24 lg:py-28">
        <div className="mx-auto max-w-[1400px]">
          {!selectedCountry ? (
            ...
            <div>
              <button type="button" onClick={resetCountry} className="mb-8 inline-flex items-center gap-2 text-sm font-bold text-[var(--fm-text-secondary)] transition-colors hover:text-[var(--fm-text-primary)]"><ArrowLeft className="h-4 w-4" /> All countries</button>
