...
export function SectionLabel({ className, children, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return <div className={cn('font-mono text-fm-label font-bold uppercase tracking-fm-label text-[var(--fm-lime)]', className)} {...props}>{children}</div>
}

export function StatusBadge({ status, className, ...props }: React.HTMLAttributes<HTMLSpanElement> & { status: 'success' | 'info' | 'warning' | 'danger' | 'neutral' }) {
  const styles = {
  ...
  return <span className={cn('inline-flex h-6 items-center gap-1.5 rounded-[var(--fm-radius-pill)] px-2.5 font-mono text-[11px] font-medium', styles[status], className)} {...props} />
}

export function IconContainer({ className, children, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return <div className={cn('flex size-10 shrink-0 items-center justify-center rounded-[var(--fm-radius-md)] border border-[var(--fm-border)] bg-[var(--fm-surface-raised)] text-[var(--fm-lime)]', className)} {...props}>{children}</div>
}

export function PageHeader({ eyebrow, title, description, className }: { eyebrow?: React.ReactNode; title: React.ReactNode; description?: React.ReactNode; className?: string }) {
  return <header className={cn('space-y-3', className)}>
    {eyebrow && <SectionLabel>{eyebrow}</SectionLabel>}
    <h1 className="text-3xl font-semibold tracking-[-0.045em] text-[var(--fm-text-primary)] sm:text-4xl">{title}</h1>
    {description && <p className="max-w-2xl text-sm leading-6 text-[var(--fm-text-secondary)] sm:text-base">{description}</p>}
  </header>
}
