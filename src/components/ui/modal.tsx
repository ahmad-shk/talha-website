        ...
        {(title || description || showClose) && (
          <header className="mb-6 flex items-start gap-4">
            <div className="min-w-0 flex-1 space-y-1.5">
              {title && <h2 id="fm-modal-title" className="text-lg font-semibold tracking-[-0.02em]">{title}</h2>}
              {description && <p className="text-sm leading-6 text-[var(--fm-text-secondary)]">{description}</p>}
            </div>
            {showClose && (
              <button
                type="button"
