      ...
      >
        <div className="flex items-center gap-3 border-b border-[var(--fm-border)] bg-[var(--fm-surface-raised)] px-4 pb-3.5 pt-4 text-[var(--fm-text-primary)]">
          <div className="relative grid h-10 w-10 flex-shrink-0 place-items-center rounded-full bg-[var(--fm-lime)] text-[var(--fm-graphite-deep)]">
            ...
            <span className="absolute -bottom-px -right-px h-[11px] w-[11px] rounded-full border-2 border-[var(--fm-surface-raised)]" style={{ background: WHATSAPP_GREEN_BRIGHT }} />
          </div>
          <div className="min-w-0 flex-1 leading-tight">
            <p className="font-display text-[15px] font-bold">Audvertax Support</p>
            <p className="mt-0.5 text-xs text-[var(--fm-text-tertiary)]">Typically replies in minutes</p>
          </div>
          <button
            type="button"
        ...
        <div className="px-4 pb-1.5 pt-[18px] bg-[radial-gradient(circle_at_0_0,var(--fm-lime-soft),transparent_60%)]">
          <div className="relative rounded-tl-[4px] rounded-tr-[var(--fm-radius-md)] rounded-bl-[var(--fm-radius-md)] rounded-br-[var(--fm-radius-md)] border border-[var(--fm-border)] bg-[var(--fm-surface-raised)] px-3.5 py-3 text-sm leading-relaxed text-[var(--fm-text-primary)] shadow-[var(--fm-shadow-subtle)]">
            <span className="mb-1 block font-display font-bold text-[var(--fm-lime-bright)]">Hi there! 👋</span>
        ...
        <div className="flex items-end gap-2 px-3 pb-3 pt-3">
          <textarea
            ...
            rows={1}
            className="max-h-24 flex-1 resize-none rounded-[var(--fm-radius-md)] border border-[var(--fm-border)] bg-[var(--fm-graphite-deep)] px-3 py-2.5 font-sans text-sm text-[var(--fm-text-primary)] outline-none transition focus:border-[var(--fm-lime)] focus:shadow-[0_0_0_3px_var(--fm-lime-soft)]"
          />
          <button
            type="button"
            ...
            style={{ background: `linear-gradient(150deg, ${WHATSAPP_GREEN_BRIGHT} 0%, ${WHATSAPP_GREEN} 100%)` }}
          >
      ...
      >
        {!open && (
          <span
            ...
            style={{ background: "rgba(30,179,88,.55)", animation: "fabPulse 2.6s ease-out infinite" }}
          />
        )}
        <span className="relative grid h-7 w-7 place-items-center">
          <MessageCircle className="col-start-1 row-start-1 h-7 w-7 transition-all duration-[var(--fm-motion-micro)] ease-[var(--fm-motion-ease)]" style={{ opacity: open ? 0 : 1, transform: open ? "rotate(90deg) scale(0.6)" : "rotate(0) scale(1)" }} />
          <X className="col-start-1 row-start-1 h-7 w-7 transition-all duration-[var(--fm-motion-micro)] ease-[var(--fm-motion-ease)]" style={{ opacity: open ? 1 : 0, transform: open ? "rotate(0) scale(1)" : "rotate(-90deg) scale(0.6)" }} />
        </span>
      </button>
    </div>
  );
