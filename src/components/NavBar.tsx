...
export default function NavBar() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Keep the mobile menu trigger reachable while scrolling on small screens.
      ...
      setNavVisible(currentScrollY < 24 || scrollingUp || mobileOpen);
      lastScrollY.current = currentScrollY;
    };
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-[100] w-full px-2.5 py-2 transition-transform duration-300 ease-out sm:sticky sm:px-6 sm:py-3.5 lg:px-8 ${navVisible ? "lg:translate-y-0" : "lg:-translate-y-[calc(100%+1rem)]"}`}>
      <nav className="mx-auto flex w-full max-w-[1400px] items-center justify-between gap-2 rounded-[var(--fm-radius-xl)] border border-[var(--fm-border)] bg-[var(--fm-graphite-deep)]/[.90] py-2 pl-3 pr-2.5 sm:gap-fm-5 sm:py-[10px] sm:pl-5 sm:pr-3.5 shadow-[var(--fm-shadow-elevated)] backdrop-blur-xl">
        <Link href="/" className="flex flex-shrink-0 items-center gap-1.5" aria-label="Audvertax home"><span className="h-5 w-7 overflow-hidden"><Image src="/audvertax_logo.png" alt="" width={130} height={35} className="h-5 w-auto max-w-none" priority /></span><span className="font-sans text-[17px] font-semibold tracking-[-0.03em] text-[var(--fm-text-primary)]">AudVertax</span></Link>
